...
const NUMBER_FIELDS: ActiveFilterField[] = [
  'id',
  'estimated_time_min',
  'estimated_time_max',
];
  ...
  for (const [key, value] of Object.entries(filters)) {
    ...
    if (k === 'search') {
      result.push({
        id: generateId(),
        field: 'search',
        operator: 'contains' as TextFilterOperator,
        value: String(value),
      });
      continue;
    }
    if (NUMBER_FIELDS.includes(k as ActiveFilterField)) {
      result.push({
        id: generateId(),
        field: k,
        operator: 'equals' as NumberFilterOperator,
...
function activeFiltersToFilterValues(
  ...
  for (const af of activeFilters) {
    if (
      ...
      if (af.operator === 'from' && af.value != null)
        out[keys.from as string] = String(af.value);
      else if (af.operator === 'to' && af.value != null)
        out[keys.to as string] = String(af.value);
    ...
    if (
      af.operator === 'between' &&
      (af.field === 'estimated_time_min' || af.field === 'estimated_time_max')
    ) {
    ...
    if (
      af.operator === 'includes' &&
      af.valueMulti &&
      af.valueMulti.length > 0
    ) {
      out[key] =
        af.valueMulti.length === 1 ? af.valueMulti[0] : af.valueMulti.join(',');
      continue;
    }
...
export function getAvailableFilters(
  ...
  if (options.search)
    list.push({
      key: 'search',
      label: FILTER_FIELD_LABELS.search || 'Search',
      kind: 'text',
    });
  if (options.statuses?.length)
    list.push({
      key: 'status_id',
      label: FILTER_FIELD_LABELS.status_id || 'Status',
      kind: 'dropdown',
      optionKey: 'statuses',
    });
  if (options.roles?.length)
    list.push({
      key: 'role_id',
      label: FILTER_FIELD_LABELS.role_id || 'Role',
      kind: 'dropdown',
      optionKey: 'roles',
    });
  if (options.priorities?.length)
    list.push({
      key: 'priority_id',
      label: FILTER_FIELD_LABELS.priority_id || 'Priority',
      kind: 'dropdown',
      optionKey: 'priorities',
    });
  if (options.types?.length)
    list.push({
      key: 'type_id',
      label: FILTER_FIELD_LABELS.type_id || 'Type',
      kind: 'dropdown',
      optionKey: 'types',
    });
  if (options.projects?.length)
    list.push({
      key: 'project_id',
      label: FILTER_FIELD_LABELS.project_id || 'Project',
      kind: 'dropdown',
      optionKey: 'projects',
    });
  if (options.users?.length) {
    list.push({
      key: 'assignee_id',
      label: FILTER_FIELD_LABELS.assignee_id || 'Assignee',
      kind: 'dropdown',
      optionKey: 'users',
    });
    if (type === 'tasks') {
      list.push({
        key: 'holder_id',
        label: FILTER_FIELD_LABELS.holder_id || 'Holder',
        kind: 'dropdown',
        optionKey: 'holders',
      });
      list.push({
        key: 'created_by',
        label: FILTER_FIELD_LABELS.created_by || 'Created By',
        kind: 'dropdown',
        optionKey: 'createdBy',
      });
    }
  }
  if (options.parent_id?.length && type === 'tasks')
    list.push({
      key: 'parent_id',
      label: FILTER_FIELD_LABELS.parent_id || 'Parent Task',
      kind: 'dropdown',
      optionKey: 'parent_id',
    });
  if (options.createdBy?.length && type === 'projects')
    list.push({
      key: 'created_by',
      label: FILTER_FIELD_LABELS.created_by || 'Created By',
      kind: 'dropdown',
  ...
  if (options.start_date_from || options.start_date_to)
    list.push({
      key: 'start_date',
      label: FILTER_FIELD_LABELS.start_date || 'Start Date',
      kind: 'date',
    });
  if (options.due_date_from || options.due_date_to)
    list.push({
      key: 'due_date',
      label: FILTER_FIELD_LABELS.due_date || 'Due Date',
      kind: 'date',
    });
  if (options.created_from || options.created_to)
    list.push({
      key: 'created',
      label: FILTER_FIELD_LABELS.created || 'Created',
      kind: 'date',
    });
  if (options.id && type === 'tasks')
    list.push({
      key: 'id',
      label: FILTER_FIELD_LABELS.id || 'ID',
      kind: 'number',
    });
  if (options.estimated_time_min || options.estimated_time_max)
    list.push({
      key: 'estimated_time_min',
      label: FILTER_FIELD_LABELS.estimated_time_min || 'Est. Time',
      kind: 'number',
    });
  if (options.showActiveOnly && type === 'tasks')
    list.push({
      key: 'inactive_statuses_only',
      label: FILTER_FIELD_LABELS.inactive_statuses_only || 'Inactive',
      kind: 'dropdown',
    });
  return list;
}
...
export const useFilterPanel = (
  ...
  const [activeFilters, setActiveFilters] = useState<ActiveFilter[]>(() =>
    filtersToActiveFilters(filters),
  );
      ...
      const same =
        ...
        prev.every((p, i) => {
          ...
          if (
            ...
            return false;
          if (
            String(p.value) !== String(n.value) ||
            String(p.value2 ?? '') !== String(n.value2 ?? '')
          )
            return false;
          const pMulti =
            (p as ActiveFilter).valueMulti?.slice().sort().join(',') ?? '';
  ...
  const getDisplayValue = (
    field: keyof FilterPanelOptions,
    value: string | number,
    optionsMap: FilterPanelOptions,
  ): string => {
    ...
    if (Array.isArray(fieldOption)) {
      const option = fieldOption.find(
        (opt: FilterOption) => String(opt.id) === String(value),
      );
  ...
  const getAppliedFilters = (optionsMap: FilterPanelOptions) => {
    return activeFilters
      ...
      .map((af) => {
        const fieldKey =
          typeof af.field === 'string' && DATE_FIELD_TO_KEYS[af.field]
            ? af.field
            : af.field;
        const displayLabel =
          FILTER_FIELD_LABELS[fieldKey as string] || fieldKey;
        let displayValue = String(af.value ?? '');
        if (af.operator === 'between' && af.value2 != null)
          displayValue = `${af.value} – ${af.value2}`;
        else if (af.field === 'inactive_statuses_only')
          displayValue = af.value ? 'Inactive' : 'Active';
        else if (
          af.operator === 'includes' &&
          af.valueMulti &&
          af.valueMulti.length > 0 &&
          ...
          if (Array.isArray(arr)) {
            displayValue = af
              .valueMulti!.map(
                (id) =>
                  arr.find((o: FilterOption) => String(o.id) === String(id))
                    ?.name ?? id,
              )
              .join(', ');
          } else displayValue = af.valueMulti!.join(', ');
        } else if (
          DROPDOWN_FIELDS.includes(af.field as ActiveFilterField) &&
          optionsMap
        ) {
          const optionKeyMap: Record<string, keyof FilterPanelOptions> = {
            status_id: 'statuses',
          ...
          if (Array.isArray(arr)) {
            const opt = arr.find(
              (o: FilterOption) => String(o.id) === String(af.value),
            );
  ...
  const getAppliedFiltersFromValues = useCallback(
    (appliedFilters: FilterValues, optionsMap: FilterPanelOptions) => {
      ...
      for (const [key, value] of Object.entries(appliedFilters)) {
        ...
        if (k === 'inactive_statuses_only')
          displayValue = value ? 'Inactive' : 'Active';
        else if (
          DROPDOWN_FIELDS.includes(k as ActiveFilterField) &&
          optionsMap
        ) {
          ...
          if (Array.isArray(arr)) {
            ...
            if (strVal.includes(',')) {
              ...
              displayValue = ids
                .map(
                  (id) =>
                    arr.find((o: FilterOption) => String(o.id) === id)?.name ??
                    id,
                )
                .join(', ');
            } else {
              const opt = arr.find(
                (o: FilterOption) => String(o.id) === String(value),
              );
              displayValue = opt?.name ?? displayValue;
            }
          }
  ...
  const updateFilter = (
    id: string,
    updates: Partial<
      Pick<ActiveFilter, 'operator' | 'value' | 'value2' | 'valueMulti'>
    >,
  ): void => {
    setActiveFilters((prev) =>
      prev.map((f) => {
        ...
        if (updates.operator === 'includes') {
          if (next.valueMulti == null)
            next.valueMulti = next.value != null ? [Number(next.value)] : [];
        }
