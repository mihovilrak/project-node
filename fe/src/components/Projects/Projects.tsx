...
const Projects: React.FC = () => {
  ...
  const childrenByParentId = useMemo(() => {
    const rootIds = new Set(
      roots.map((r) => r?.id).filter((id): id is number => id != null),
    );
      ...
      >
        {hasSubprojects && (
          <IconButton
            ...
            aria-label={
              isExpanded ? 'Collapse subprojects' : 'Expand subprojects'
            }
            sx={{ p: 0.25 }}
          >
            {isExpanded ? (
  ...
  return (
    <Box sx={{ width: '100%', p: 3 }} data-testid="projects-container">
      ...
      {error ? (
        <Typography color="error" data-testid="projects-error">
          {error}
        </Typography>
      ) : loading ? (
        ...
        <Grid
          container
          spacing={2}
          sx={{ mt: 2, width: '100%', maxWidth: 1400 }}
        >
                      ...
                      >
                        {subprojects.map((sub) => (
                          <Box
                            key={sub?.id}
                            sx={{ mb: 0.5, pl: 1 }}
                            data-testid={`project-card-${sub?.id}`}
                          >
