...
describe('WatcherList', () => {
  ...
  it('calls onRemoveWatcher when delete button is clicked', () => {
    ...
    const deleteButtons = screen.getAllByRole('button', {
      name: /remove watcher/i,
    });
    fireEvent.click(deleteButtons[0]);
    expect(defaultProps.onRemoveWatcher).toHaveBeenCalledWith(
      mockWatchers[0].user_id,
    );
  });
  ...
  it('hides manage buttons when canManageWatchers is false', () => {
    renderWithRouter(
      <WatcherList {...defaultProps} canManageWatchers={false} />,
    );
    expect(screen.queryByText(/manage watchers/i)).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /remove watcher/i }),
