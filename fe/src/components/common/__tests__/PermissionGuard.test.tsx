...
describe('PermissionGuard', () => {
  ...
  it('shows children when loading but showLoading is false', () => {
    ...
    expect(
      screen.queryByText('Loading permissions...'),
  ...
  it('shows children when permission is granted', () => {
    ...
    expect(screen.getByText('Protected Content')).toBeInTheDocument();
    expect(
      screen.queryByText("You don't have permission to view this content"),
    ).not.toBeInTheDocument();
  });
  ...
  it('handles custom loading component when provided', () => {
    ...
    render(
      <PermissionGuard {...defaultProps} loadingComponent={loadingComponent} />,
    );
